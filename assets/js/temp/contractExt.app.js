...
$(window).load(function() {
	...
	var liHtml = "<i class='fa fa-exclamation-triangle'></i> If your contractor is in Beeline contact Emily Rotary or Sheri Pallas.";				
		...
		$().SPServices({
		  ...
		  completefunc: function (xData, Status) {
			...
			if (firstName != null && firstName !=""){
				liHtml = liHtml + "<i class='fa fa-exclamation-triangle'></i> If your contractor is in Beeline contact Emily Rotary or Sheri Pallas.";
...
function requestType(){
		...
		$("#userProgress").append("<li id='selectedSub0'>NEW contract end date: <i class='fa fa-calendar'></i> " + document.getElementById("myMonth").value + "/" + document.getElementById("myDay").value + "/" + document.getElementById("myYear").value + " <a href='#contact' id='changeMe0'>Change</a></li>");	
...
function changeBack(step){	
   ...
   $('form#contactForm').on('submit', function(event) {
	  ...
	  // });
		...
		today = new Date(document.getElementById("myYear").value + "-" + document.getElementById("myMonth").value + "-" + document.getElementById("myDay").value + "T12:00:00");
		
		//var varDate = today.format("yyyy-mm-dd'T'HH:MM:ss");
		
		
      myID = AddListItem(ContName,altNativeId,today.format("yyyy-mm-dd'T'HH:MM:ss'Z'"),managerId);
	...
	function AddListItem(name,nativeid,thedate,managerid){
		var newId = "";
		...
		$().SPServices({ 
			 ...
			 webURL: "/sites/help/",
			 batchCmd: "New", 
			 listName: "{CB4A25F0-0502-42C3-9720-D8D5BDD0E29E}", 
			 valuepairs: [["Title", name], ["Contractors_x0020_Native_x0020_I", nativeid], ["New_x0020_Expiration_x0020_Date", thedate], ["Approved_x0020_By", managerid]], 
			 completefunc: function(xData, Status) { 
...
function populateStatus(){		
	...
	var liHtml = "";
	 ...
	 $().SPServices({
		...
		webURL: "/sites/help/",
		listName: "{CB4A25F0-0502-42C3-9720-D8D5BDD0E29E}",
		viewName: "{ED6E95AF-0A08-450E-934D-B1AD77DFC96C}",
		CAMLViewFields: "<ViewFields><FieldRef Name='ID' /><FieldRef Name='Title' /><FieldRef Name='Contractors_x0020_Native_x0020_I' /><FieldRef Name='New_x0020_Expiration_x0020_Date' /><FieldRef Name='Approved_x0020_By' /><FieldRef Name='ProcessStatus' /><FieldRef Name='ProcessErrMsg' /></ViewFields>",
		completefunc: function (xData, Status) {
		  ...
		  $(xData.responseXML).SPFilterNode("z:row").each(function() {
			...
			liHtml = liHtml + "<h1>" + $(this).attr("ows_Title") + "</h1>";
			
			var altNativeID = $(this).attr("ows_Contractors_x0020_Native_x0020_I");
			...
			var frmDate = $(this).attr("ows_New_x0020_Expiration_x0020_Date");
			frmDate = frmDate.substr(frmDate.indexOf("-")+1,5) + "-" + frmDate.substr(0,frmDate.indexOf("-"));
			
			liHtml = liHtml + "<p>NEW Expiration Date: " + frmDate + "</p>";
			liHtml = liHtml + "<div id='message-status'>";
			
			var theStatus = $(this).attr("ows_ProcessStatus");
			
			if (theStatus == "Not Started") {
				liHtml = liHtml + "<i class='fa fa-times'></i><br /><b>Status:</b> " + theStatus + "<br />";
			}
			else if (theStatus == "In Progress") {
				liHtml = liHtml + "<i class='fa fa-spinner'></i><br /><b>Status:</b> " + theStatus + "<br />";
			}
			else if (theStatus == "Complete") {
				liHtml = liHtml + "<i class='fa fa-check'></i><br /><b>Status:</b> " + theStatus + "<br />";
			}
			else if (theStatus == "Error") {
				liHtml = liHtml + "<i class='fa fa-exclamation-triangle'></i><br /><b>Status:</b> " + theStatus + "<br />";
				liHtml = liHtml + "<p>" + $(this).attr("ows_ProcessErrMsg") + "</p>";
			}
			
			liHtml = liHtml + "<b>Request ID:</b> " + $(this).attr("ows_ID") + "<br />";
			
			var Upline = $(this).attr("ows_Approved_x0020_By");
			...
			if (Upline != null){
			   ...
			   $().SPServices({
				...
				liHtml = liHtml + "<b>Upline:</b> " + Manager + ", " + Upline + "<br />";
			}
